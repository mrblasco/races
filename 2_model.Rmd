...
def: A contest is a N player game. Players move simultaneously and select a performance variable $y_i$  (the xxxx in performing a task) and a timing $t_i$ (to complete a task), both being nonnegative numbers. Players are ranked according to their choices and a player's rank is given by the function $r_i(y_1,...y_N, t_1, ..., t_N)$. The top $K$ ranked players (e.g., $r_i<K$) are awarded a prize of value $V_1 > V_2 > ... > V_K$.


The goal for each player is to maximize the expected utility. If the timing is above a given deadline $d>0$ or the performance is below a certain level $q>0$, the player gets zero utility. Otherwise, the player is given a rank based on his performance and timing. 
