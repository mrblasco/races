...
//Tests experience voting method run by getAnnotationAtAllExp in Aggregator.java

public class TestExperienceVoting {
	...
	public static void main(String[] args) throws XMLStreamException, IOException {	
		...
		executeVotingExperience(mturkfile, goldfile, k_dir);
	}
	
	public static void executeVotingExperience(String mturkfile, String goldfile, String k_dir) throws XMLStreamException, IOException{
		if(goldfile==null){
		...
		Map<Double,List<Annotation>> exp_annos = agg.getAnnotationAtAllExp(mturk_annos);
		for(Double exp : exp_annos.keySet()){
			List<Annotation> annos = exp_annos.get(exp);
			if(goldfile!=null){
				//execute comparison versus gold, report results
				ComparisonReport report = ac.compareAnnosCorpusLevel(gold_annos, annos, "K="+exp);
				System.out.println("Exp="+exp+"\t"+report.getRow());
			}
