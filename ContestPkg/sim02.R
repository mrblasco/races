...
curve(ca, ylim=c(0, 4)); curve(ct, add=TRUE, lty=2); curve(cy, add=TRUE, lty=3)

# Expected payoffs
curve(epayoff(x, v, 12))
curve(epayoff(x, v, 6), add=TRUE, lty=2)
curve(epayoff(x, v, 3), add=TRUE, lty=3)

# Expected utility
curve(u(x, y=0, ti=0, v=v, rivals=12))

# Bids
curve(y.tournament(x, v, 12))

u(0.2, y.tournament(0.2, v, 2), ti=1, v=v, rivals=2)
u(0.4, y.tournament(0.4, v, 2), ti=1, v=v, rivals=2)
u(0.8, y.tournament(0.8, v, 2), ti=1, v=v, rivals=2)
u(1, y.tournament(1, v, 2), ti=1, v=v, rivals=2)
