...
\date{Last updated: 13 February, 2017}
...
Open question. What happens when \(\target\) in a tournament is higher
than in a race.

Conjecture. Suppose the contest designer can choose the target that max
profits in both the race and the tournament. Then, the optimal
\(\target\) in tournament should be lower than that in a race.

Proof. Suppose that \(\target\) was higher, then the optimal design of a
race would be to increase the target \(\target\) \ldots{}. but
computations are still a bit complicated.
