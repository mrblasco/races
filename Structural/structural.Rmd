...
In binomial linear models, the probability function $p(x, \theta)$ is related to a linear prediction through a one-to-one transformation called the link function (see xxxx). 

In the MS theoretical approach, our preliminary analysis of the model showed that the predictor is not generally linear and it is modeled as follows:

\begin{equation}
	\label{probability}
	p(x_l, \theta) = 1 - F_{\theta}(\mtype)
\end{equation}

where $F_{\theta}$ is a parametric distribution function of a latent ability variable that is privately observed by each player with $\theta$ being a vector of distributional parameters and $\mtype$ being the marginal type of the room. 

In each room, the marginal type is then determined by the following zero profit condition:

\begin{equation}
	\label{zero profit}
	R(\mtype) = \cability(\mtype)\cscore(\target)\ctime(\deadline)
\end{equation}
where the $R(\mtype)$ is the expected revenue of the marginal type must be equal to the  costs. The expected revenue is
\begin{equation}
	R(\mtype) = 
...
Because abilities are drawn at random and independently, the distribution of the order statistics on the left hand side depends on the distribution $F_{\theta}$ and, hence, it is known up to the parameters $\theta$ that we want to estimate from the data. 

Also, the cost function of the marginal type is evaluated at the target and the deadline, which are determined by the room characteristics $x$ (e.g., the problem, whether there's a deadline or not, whether's there's a minimum entry requirement or not). We assume the following linear relationship:
\begin{equation}
...
```
# Define likelihood
structreg <- function(x, y, wt = rep(1, length(y)), intercept = T, start = rep(0, p), ...) {
	fmin <- function(beta, X, y, w) {
		# Suppose uniform with 10 competitors
		p <- 1 - (beta[1] * X)^(1/10)  # Function of parameters
...
n <-10
n.rooms <- 500
X <- rchisq(n.rooms, df=3)
eta <- 0.05
p <- function(eta, X) {
	prob <- 1 - (eta * X)^{1/n}
	ifelse(prob>0, prob, 0.05)
}
x.P <- p(eta, X)
y <- sapply(x.P, function(x) rbinom(n=1, size=n, prob=x))
