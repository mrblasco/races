...
Flow: import data > make technically correct > make consistent. 
...
svy.ini.correct <- function(x) {
  ...
  x$country_origin <- gsub("^[A-Z]{2} - ", "", x$country_origin)
...
svy.end.correct <- function(x) {
  ...
  names(x)[grep("risk", names(x))] <- "risk2"
  ...
  x$finished <- as.logical(x$finished==1)
  x$handle <- standardize.handle(x$handle)
  x$risk2 <- as.numeric(gsub("[^0-9]","", as.character(x$risk2)))
...
```{r}
# Check any handles with submissions not assigned to a treatment
any(!levels(subs$handle) %in% levels(assignment$handle))

# Check any handles assigned to a treatment but not in initial survey
sum(!levels(assignment$handle) %in% levels(svy.ini$handle)) # 19!!!

# Check any handles in final survey but not assigned to treatment
i <- which(!levels(svy.end$handle) %in% levels(assignment$handle)) # 6!!!
levels(svy.end$handle)[i]
```
