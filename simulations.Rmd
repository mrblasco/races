...
#-------------------------------------------------#
f <- function(n, ca, cy, ct, v,
			, xlim=c(0, Inf), ylim=c(0, Inf), tlim=ylim
			, p=plnorm, d=dlnorm, r=rlnorm
			, deadline=NULL, target=NULL
			, ...) {
	# Checks ********************************#
	if (any(v<0.5 | v>1)) stop("V must be higher then 0.5 and less than 1.")
	if (is.null(deadline)) deadline <- tlim[2]
	if (is.null(target)) target <- ylim[2]
	# PDF of the k'th order statistic of n iid X's
	dord <- function(x, k, n, ...) {
	...
	# CDF of the k'th order statistic of n iid X's
	pord <- function(x, k, n, ...) {
		z <- 0
		for (j in k:n) {
	...
	#	V: prize
	#	requires cy, ct, pord
	u <- function(x, y, ti, v, n, ...) {
		x <- ifelse(x==0, .Machine$double.xmin, x)
		k <- n
		costs <- x^ca * y^cy * ti^ct
