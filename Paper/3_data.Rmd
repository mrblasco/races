...
# Factors
vars <- c("country_name", "age", "gender", "educ", "plang")
foo <- sapply(races[, vars], function(x) summary(x, maxsum=6)) ## get summary of each var
foo.test <- sapply(races[, vars], function(x) chisq.test(table(x, races$treatment) + 1, simulate.p.value=TRUE)$p.val)

bar <- xtable.factors(foo, foo.test)
align(bar) <- c("{}l","l","p{4cm}","r","r")
render.xtable(bar, "Descriptive statistics", "tab: desc", include.rownames=FALSE, notes="This table shows the frequency for each response category of five categorical variables for country of residence, age, gender, highest degree achieved, and preferred programming language. For each variable, a Pearson's Chi-squared test shows no evidence of dependence  between the variable and the treatments. P-values are reported in the last column.")

# Table with numeric variables
vars <- c("Algo rating"="algorating"
...
# foo <- sapply(races[, vars], function(x) summary(x)) ## get summary of each var
# foo.test <- sapply(races[, vars], function(x) kruskal.test(split(x, races$treatment))$p.val)
# xtable.factors(foo, foo.test)

stargazer(races[, vars], header=FALSE, title='Descriptive statistics', covariate.labels=names(vars), digits=1)
```
...
as.numeric(difftime(start_date, races$create_date))/7 -> week_diff

# Past competitions (experience)
quantile(races$mmevents, p=c(0.1, 0.9)) -> rounds_qtl

# Individual ratings (skills)
quantile(races$mmrating, p=c(0.1, 0.9), na.rm=TRUE) -> rating_qtl
```
...
```{r}
library(car)
spm(~ mmrating+mmevents+algorating+algoevents, data=races, pch=16)
```
...
library(maps)
map()
with(races, points(locationlongitude, locationlatitude, pch=21, col='red'))
```