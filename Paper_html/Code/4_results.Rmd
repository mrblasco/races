...
```{r}
day <- as.numeric(format(subs$sub_time, '%d'))
subs$day <- cut(day, c(8,10,12,14, 16), include.lowest=T, right=FALSE)
subs.long0 <- expand.grid(handle=levels(subs$handle), day=levels(subs$day))
subs.long1 <- aggregate(sub_id ~ day + handle, data=subs, FUN=length)
subs.long <- merge(subs.long0, subs.long1, by=c("handle", "day"), all.x=TRUE)
subs.long$sub_id[is.na(subs.long$sub_id)] <- 0
plot(subs.long[, -1])
summary(glm(sub_id ~ day, data=subs.long))
summary(glm(sub_id ~ day, data=subs.long, family=quasipoisson))
summary(glm(sub_id>0 ~ day, data=subs.long))
```
