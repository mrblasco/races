...
#' Contest simulation
contest <- function(x, n, type, prize=c(1, 0), cost=c(-1, 2, -1)
	...
	p2 <- (1 - p1) * pord(x, k=rivals-1, n=rivals-1, p=p, d=d)
	
	# Expected payoff
	E <- prize[1] * p1 + prize[2] * p2
	cost0 <- cx(x) * cy(target) * ct(deadline)
	if (all(E < cost0)) stop("No entrants")
	...
	# Internal functions	
	A <- function(x, ...) dord(x, rivals, rivals, p, d, ...) / cx(x)
	B <- function(x, ...) {
		(-dord(x, rivals, rivals, p, d, ...) * pord(x, rivals-1, rivals-1, p, d, ...) +
		dord(x, rivals-1, rivals-1, p, d, ...) * (1-pord(x, rivals, rivals, p, d, ...))) / cx(x)
	}
	...
	b0 <- cy(target)
	prize.scaled <- prize / ct(deadline)
	...
	bid.linear <- b0 + prize.scaled[1]  * v1 + prize.scaled[2] * v2
	bid <- ifelse(x < low.type, 0, cy.inv(bid.linear))
	cost.eq <- cx(x) * cy(bid) * ct(deadline)
	cbind(E, cost.eq)
	
	cy'(y) y'(x)  ct(deadline)
