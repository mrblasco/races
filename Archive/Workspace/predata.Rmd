...
tc.profiles <- read.csv("Data/details_profile.csv")
submissions <- read.csv("Data/submissions.csv")
survey.ini  <- read.csv("Data/qualtrics/SV_8elNruzKCgPB72B.csv"
              , skip=1, na.strings=c("","NA")) 
survey.end  <- read.csv("Data/qualtrics/SV_0DSqL7Exk0A7Bdj.csv"
              , skip=1, na.strings=c("","NA"))
assignment <- rbind(read.csv("Data/race.csv")
                    , read.csv("Data/tournament.csv")
                    , read.csv("Data/reserve.csv"))
...
# Web profiles
fnames <- list.files(c("Data/profiles-handles","Data/profiles"), full.names=TRUE)
profiles <- NULL
...
```{r}
save(list=c("dat", "scores"), file='races.RData')
```
